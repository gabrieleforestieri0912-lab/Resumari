...
const codeExamples = {
  curl: [
    "curl -X POST https://resumari.vercel.app/api/v1/transcript \\",
  ...
  bulkCurl: [
    "curl -N -X POST https://resumari.vercel.app/api/v1/transcript/bulk \\",
    '  -H "Content-Type: application/json" \\',
    '  -H "X-API-Key: rsm_live_YOUR_KEY_HERE" \\',
...
const errors = [
  {
    ...
    description: "Intestazione X-API-Key non fornita",
  },
  { code: "invalid_api_key", description: "Chiave non valida o revocata" },
  {
    code: "invalid_input",
    description: "Corpo della richiesta o URL YouTube non valido",
  },
  { code: "insufficient_credits", description: "Crediti insufficienti" },
  {
    code: "no_transcript",
    description: "Il video non ha sottotitoli disponibili",
  },
  {
    code: "resolution_failed",
    description: "URL canale/playlist non risolvibile",
  },
...
const faqs = [
  ...
  },
  {
    ...
    a: "La risposta include: video_id, titolo, canale, durata, transcript (array di segmenti con testo, start time e durata), testo completo, lingua, crediti usati e rimanenti.",
  },
  {
    q: "Quali sono i rate limit?",
    a: "30 richieste al minuto per chiave API. Per volumi superiori, contattaci per un piano personalizzato.",
  },
  {
    q: "Cosa succede se un video non ha transcript?",
    a: "La richiesta non va a buon fine e i crediti non vengono consumati. Le estrazioni fallite vengono rimborsate automaticamente.",
  },
  {
    q: "Posso specificare una lingua?",
    a: "Al momento l'API tenta prima l'italiano, poi l'inglese. Il rilevamento è automatico.",
  },
  {
    q: "Come mi autentico?",
    a: "Includi la tua chiave API nell'intestazione X-API-Key di ogni richiesta. Non serve OAuth.",
  },
  {
    q: "Devo usare una chiave YouTube Data API?",
    a: "No. Resumari gestisce l'estrazione senza bisogno della YouTube Data API.",
  },
];
...
export default function ApiKeysPage() {
  ...
  const [expandedEndpoint, setExpandedEndpoint] = useState<
    "single" | "bulk" | null
  >("single");
  const [expandedCodeLang, setExpandedCodeLang] = useState<
    "curl" | "python" | "node"
  >("curl");
  const [expandedBulkLang, setExpandedBulkLang] = useState<
    "curl" | "python" | "node"
  >("curl");
  ...
  async function revokeKey(id: string) {
    if (
      !confirm(
        "Revocare questa chiave? Le integrazioni che la usano smetteranno di funzionare.",
      )
    )
      return;
    const token = localStorage.getItem("token");
    ...
    <div className="min-h-screen bg-white dark:bg-zinc-950">
      ...
      <section className="pt-32 pb-20 px-4">
        <div className="max-w-5xl mx-auto mb-6">
          <Breadcrumb
            items={[{ label: "Home", href: "/" }, { label: "API Keys" }]}
          />
        </div>
        <div className="max-w-5xl mx-auto text-center">
          ...
          <p className="text-gray-400 dark:text-gray-500 max-w-2xl mx-auto mb-10">
            Molte API di trascrizione richiedono abbonamenti o sono limitate a
            un video singolo. L&apos;API di Resumari estrae dozzine di
            trascrizioni in una singola chiamata, con crediti pay-as-you-go che
            non scadono mai.
          </p>
          ...
          <div className="max-w-lg mx-auto bg-gray-50 dark:bg-zinc-900 rounded-3xl p-8 border border-gray-100 dark:border-zinc-800 shadow-sm">
            ...
            {newlyCreatedKey && (
              <div className="mt-4 bg-yellow-50 dark:bg-yellow-950/40 border border-yellow-200 dark:border-yellow-800 rounded-xl p-4">
                <p className="text-xs font-bold text-yellow-700 dark:text-yellow-300 uppercase mb-1">
                  Chiave creata — copiala ora!
                </p>
                ...
                <p className="text-xs text-yellow-600 dark:text-yellow-300 mt-2">
                  Questa chiave viene mostrata una volta sola. Salvala in un
                  posto sicuro.
                </p>
              </div>
            )}
      ...
      <section className="py-16 px-4 bg-gray-50 dark:bg-zinc-900">
        <div className="max-w-4xl mx-auto">
          ...
          <p className="text-gray-500 dark:text-gray-400 text-center mb-12 max-w-xl mx-auto">
            Ottieni la tua chiave API e inizia a estrarre trascrizioni in meno
            di un minuto. I video devono avere sottotitoli YouTube esistenti.
          </p>
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {[
              ...
              },
              {
                ...
                desc: "Vai alla sezione API Keys qui sopra, nomea la tua chiave e copiala. La chiave viene mostrata una volta — salvala in modo sicuro.",
              },
              {
                step: "3",
                icon: Terminal,
                title: "Fai la tua prima richiesta",
                desc: "Invia una richiesta POST con la tua chiave nell'intestazione X-API-Key e un video ID. Riceverai il JSON con la trascrizione completa.",
              },
            ].map((item, i) => (
              <div
                key={i}
      ...
      <section className="py-16 px-4">
        <div className="max-w-4xl mx-auto">
          ...
          <p className="text-gray-500 dark:text-gray-400 mb-6">
            ...
            <code className="bg-gray-100 dark:bg-zinc-800 px-2 py-0.5 rounded text-sm font-mono">
              X-API-Key
            </code>{" "}
            in ogni richiesta.
          </p>
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-gray-100 dark:border-zinc-800 shadow-sm overflow-hidden mb-6">
            <button
              onClick={() =>
                setExpandedEndpoint(
                  expandedEndpoint === "single" ? null : "single",
                )
              }
            ...
            >
              <div>
                <div className="flex items-center gap-3 mb-1">
                  <span className="px-2.5 py-0.5 bg-green-100 dark:bg-green-900/40 text-green-700 dark:text-green-300 text-xs font-bold rounded-md">
                    POST
                  </span>
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400">
                  Estrai la trascrizione da un singolo video YouTube. Costa 2
                  crediti.
                </p>
              </div>
          ...
          <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-gray-100 dark:border-zinc-800 shadow-sm overflow-hidden">
            <button
              onClick={() =>
                setExpandedEndpoint(expandedEndpoint === "bulk" ? null : "bulk")
              }
              className="w-full flex items-center justify-between p-6 hover:bg-gray-50 dark:hover:bg-zinc-800 transition-all text-left"
            >
              <div>
                <div className="flex items-center gap-3 mb-1">
                  ...
                  <code className="text-sm font-mono text-gray-800 dark:text-gray-100">
                    /api/v1/transcript/bulk
                  </code>
                </div>
                <p className="text-sm text-gray-500 dark:text-gray-400">
                  Estrazione bulk da un canale o playlist. Stream di risultati
                  via SSE. 2 crediti per video (fallimenti rimborsati).
                </p>
              </div>
                  ...
                  <div className="px-6 pb-6">
                    ...
                    <div className="bg-gray-900 dark:bg-black rounded-xl p-5 overflow-x-auto">
                      <pre className="text-sm text-gray-300 font-mono whitespace-pre">
                        {
                          codeExamples[
                            `bulk${expandedBulkLang.charAt(0).toUpperCase() + expandedBulkLang.slice(1)}` as keyof typeof codeExamples
                          ]
                        }
                      </pre>
                    </div>
...
data: {"stats":{"total":21,"succeeded":20,"failed":1}}`}</pre>
  ...
  "credits_remaining": 98
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="grid md:grid-cols-2 gap-3">
            {errors.map((err, i) => (
              <div
                key={i}
                className="bg-white dark:bg-zinc-900 rounded-xl px-5 py-4 border border-gray-100 dark:border-zinc-800 shadow-sm"
              >
          ...
          <div className="mt-10 bg-white dark:bg-zinc-900 rounded-2xl p-6 border border-gray-100 dark:border-zinc-800 shadow-sm">
            <h3 className="font-bold text-gray-900 dark:text-gray-100 mb-3">
              Documentazione API importabile
            </h3>
            <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">
              Usa l&apos;API per video che hanno già sottotitoli YouTube.
              Resumari restituisce JSON strutturato per video singoli e
              Server-Sent Events per job bulk su canali o playlist.
            </p>
            <div className="flex flex-wrap gap-3">
              {[
                ...
                <span
                  key={i}
                  className="px-4 py-2 bg-purple-50 dark:bg-purple-950/40 text-purple-700 dark:text-purple-300 text-sm font-bold rounded-xl"
                >
                  {item}
                </span>
              ))}
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-200 dark:border-zinc-700">
                  <th className="text-left py-3 px-4 font-bold text-gray-900 dark:text-gray-100">
                    API
                  </th>
                  <th className="text-left py-3 px-4 font-bold text-gray-900 dark:text-gray-100">
                    Modello
              ...
              <tbody>
                {comparisons.map((c, i) => (
                  <tr
                    key={i}
                    className={`border-b border-gray-100 dark:border-zinc-800 ${i === 0 ? "bg-purple-50 dark:bg-purple-950/40" : ""}`}
                  >
      ...
      <section className="py-16 px-4 bg-gray-50 dark:bg-zinc-900">
        <div className="max-w-4xl mx-auto">
          <h2 className="text-3xl font-black text-gray-900 dark:text-gray-100 text-center mb-12">
            Perché gli sviluppatori scelgono Resumari
          </h2>
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[
              {
                icon: Zap,
                title: "Estrazione bulk",
                desc: "Estrai dozzine di trascrizioni in una chiamata API. Processa playlist, crea dataset o controlla canali — senza scrivere un loop. I fallimenti vengono rimborsati automaticamente.",
              },
              {
                icon: Clock,
              ...
              {
                icon: Terminal,
                title: "API REST semplice",
                desc: "Una richiesta POST per video. SSE streaming per bulk. Passa un URL o ID video, ottieni il transcript completo con timestamp. Niente OAuth, niente YouTube Data API key.",
              },
              {
                icon: FileText,
                title: "Timestamp su ogni segmento",
                desc: "Ogni segmento del transcript include start time e durata. Crea file di sottotitoli, indici di ricerca o dataset time-coded da JSON strutturato.",
              },
              {
                icon: RefreshCw,
                title: "Rimborsi automatici in caso di errore",
                desc: "Alcuni video non hanno sottotitoli. Quando l'estrazione fallisce, il tuo credito viene rimborsato istantaneamente — nessun ticket di supporto.",
              },
              {
                icon: Key,
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {[
              ...
              },
              {
                ...
                icon: Key,
                title: "Genera una API Key",
                desc: "Vai alla pagina API Keys, dai un nome alla tua chiave e copiala. La chiave viene mostrata una volta — salvala in modo sicuro.",
              },
              {
                num: "3",
                icon: Terminal,
                title: "Fai la tua prima richiesta",
                desc: "Invia una richiesta POST con la tua chiave API nell'intestazione X-API-Key e un video ID. Ricevi JSON con la trascrizione completa.",
              },
            ].map((item, i) => (
      ...
      <section className="py-20 px-4">
        <div className="max-w-2xl mx-auto text-center">
          ...
          <p className="text-gray-500 dark:text-gray-400 mb-8">
            Pay As You Go, nessun abbonamento di default. I crediti non scadono
            mai.
          </p>
          <div className="flex flex-wrap justify-center gap-4">
            <button
              onClick={() =>
                document
                  .getElementById("api-key-section")
